...
class Appointment(models.Model):
    ...
    time = models.CharField(max_length=2, choices=HORARIO_CHOICES, default="AM", null=True, blank=True)  
...
def validate_unique_time_slot(sender, instance, **kwargs):
    existing_appointment = (
        Appointment.objects.filter(
            mechanic=instance.mechanic, date=instance.date, time=instance.time
        )
...
class Checklist(models.Model):
    appointment = models.OneToOneField(
        Appointment, on_delete=models.CASCADE, related_name="checklist"
    )
